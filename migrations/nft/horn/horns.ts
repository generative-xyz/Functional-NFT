...
class HORNS {
    ...
    async deployUpgradeable(name: string, symbol: string, adminAddress: any, paramAdd: any,
        ...
        console.log("HORNS deployed at proxy:", proxy.address);
        return proxy.address;
    }

    async upgradeContract(proxyAddress: any) {
        const contractUpdated = await ethers.getContractFactory("HORNS");
        console.log('Upgrading HORNS... by proxy ' + proxyAddress);
        const tx = await upgrades.upgradeProxy(proxyAddress, contractUpdated);
        console.log('HORNS upgraded on tx address ' + tx.address);
        return tx;
    }

    getContract(contractAddress: any, contractName: any = "./artifacts/contracts/nft/HORNS.sol/HORNS.json") {
