...
class AVATARS {
    ...
    async changeBaseURI(contractAddress: any, baseuri: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce
        const fun = temp?.nftContract.methods.changeBaseURI(baseuri);
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
    ...
    async setLimit(contractAddress: any, limit: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.setLimit(limit);
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
    ...
    async pause(contractAddress: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.pause();
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
    ...
    async unpause(contractAddress: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.unpause();
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
    ...
    async mint(contractAddress: any, fee: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce
    ...
    async mintWhitelist(contractAddress: any, fee: any, gas: number) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.mintWhitelist();
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
